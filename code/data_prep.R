...
create.referral.data<- function(){
  pob.full <- read.spss("Data/44102_10yrs_fup with age groups.sav", to.data.frame=T) # load the data 
  pob.full <- pob.full[pob.full$lftd < 59 & trimws(pob.full$cyt1_dia) != "Pap 0", ] #pob.full$lftd < 59 & 
  #pob.full <- pob.full[pob.full$cyt1_hr %in% c("+", "-"), ] # remove "bad" HPV test results 
  pob.full$cyt1_hpvtype <- trimws(pob.full$cyt1_hpvtype) # remove white space from HPV type columns
  pob.pos <- pob.full[pob.full$cyt1_hr =="+",] # filter to include only HPV positives 
  
  #pob.pos <- pob.pos[-which(trimws(pob.pos$cyt1_dia)== 'Pap 0'),] # remove 2 women with missing baseline results 
  ...
  baseline.cyt <- case_when(
    grepl(paste(c('Pap 1'), collapse="|"), pob.pos$cyt1_dia) ~"NILM",
    grepl(paste(c('Pap 3a2', 'Pap 3b', '4', '5'), collapse="|"), pob.pos$cyt1_dia) ~ "HSIL",
    grepl(paste(c('Pap 2', 'Pap 3a', 'Pap 3a1'), collapse="|"), pob.pos$cyt1_dia) ~"ASC-US/LSIL")
  ...
  first.repeat.cyt.cat <- case_when(
    grepl(paste(c('Pap 1', 'Pap 0'), collapse="|"), first.repeat.cyt.result) ~"NILM",
    grepl(paste(c('Pap 2', 'Pap 3a', 'Pap 3a1', 'Pap 3a2', 'Pap 3b', 'Pap 4', 'Pap 5'), collapse="|"), first.repeat.cyt.result) ~"ASC-US/LSIL+",
    TRUE ~ "Missing")
  ...
  second.repeat.cyt.cat <- case_when(
    second.repeat.cyt.result =="Pap 1" ~ "NILM",
    second.repeat.cyt.result > "Pap 1" ~ "ASC-US/LSIL+",
    TRUE ~'Missing')
  second.repeat.cyt <- ifelse(second.repeat.cyt.cat =="ASC-US/LSIL+", TRUE, FALSE)
  ...
  second.round.cyt.cat <- case_when(
    grepl(paste(c('Pap 0','Pap 1'), collapse="|"), second.round.cyt) ~"NILM",
    grepl(paste(c('Pap 3a2', 'Pap 3b', '4', '5'), collapse="|"), second.round.cyt) ~ "HSIL",
    grepl(paste(c('Pap 2', 'Pap 3a', 'Pap 3a1'), collapse="|"),second.round.cyt) ~"ASC-US/LSIL", 
    TRUE~"Missing")
  
  # hierarchical genotype grouping 
  genotype.data <- case_when(
    hpv.1618pos ~ "HPV 16/18",
    hpv.genotype.pos ~ "HPV 31/33/45/52/58",
    TRUE ~ "Other"
  )
...
hist.groups <- function(x){
  return(case_when(
    trimws(x) %in% c("Aden.Cis", "ACIS", "UE ACIS") ~ "ACIS",
    trimws(x) %in% cin3plus.codes ~ "CIN3+ not ACIS",
...
referral.data.raw$baseline.group <- case_when(
  referral.data.raw$baseline.cyt =='ASC-US/LSIL' ~'BMD',
  (referral.data.raw$baseline.cyt=='NILM' & referral.data.raw$grp=='i') ~ "Normal (i)",
  (referral.data.raw$baseline.cyt=='NILM' & referral.data.raw$grp=='c' & !referral.data.raw$hpv.genotype.pos) ~ "Normal (c)",
  T ~ NA
)
...
illegal.bmd.cases <- referral.data.raw[referral.data.raw$first.repeat.cyt=='Missing' & referral.data.raw$first.round.cin3plus==1 & referral.data.raw$baseline.cyt =='ASC-US/LSIL',]$id
legal.bmd.cases <- referral.data.raw[referral.data.raw$first.repeat.cyt!='Missing' & referral.data.raw$first.round.cin3plus==1 & referral.data.raw$baseline.cyt =='ASC-US/LSIL',] # adhere protocol
referral.data.raw[referral.data.raw$id %in% illegal.bmd.cases,]$first.repeat.cyt <- ifelse(rbern(length(illegal.bmd.cases), prop.table(table(legal.bmd.cases$first.repeat.cyt))[1]), "ASC-US/LSIL+", "NILM")

illegal.nilm.cases <-  referral.data.raw[referral.data.raw$first.repeat.cyt=='Missing' & referral.data.raw$first.round.cin3plus==1 & referral.data.raw$baseline.cyt =='NILM',]$id
